...
const limiter = rateLimit({ windowMs: 60000, max: 30, message: { error: 'Muitas tentativas.' } });
app.use('/api/', limiter);
const users = [
...
// OQ46Z2-end
// OQ48A-v2-mount
if(typeof _oqCluster==="function"){ try{ const _auth = (typeof requireAuth==="function"?requireAuth:(typeof authMiddleware==="function"?authMiddleware:(req,res,next)=>next())); _oqCluster(app, _auth); console.log("[OQ48A] /api/cluster montado"); }catch(e){ console.error("[OQ48A] mount fail", e.message);} }
if(typeof _oqBlocos==="function"){ try{ const _ab=(typeof requireAuth==="function"?requireAuth:(typeof authMiddleware==="function"?authMiddleware:(req,res,next)=>next())); _oqBlocos(app, _ab); console.log("[OQ58A] /api/blocos montado"); }catch(e){ console.error("[OQ58A] mount fail", e.message);} }
...
// B342R_MODELS_ROUTE
app.get('/api/providers/:id/models', authMiddleware, async (req,res)=>{ try{ const r=await Providers.listModels(req.params.id); res.json(r); }catch(e){ res.status(400).json({error:e.message}); } });
